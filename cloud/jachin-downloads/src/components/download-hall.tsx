    ...
    <div className="mx-auto max-w-4xl px-4 pb-20 pt-24">
      <header className="mb-10 flex flex-col gap-4 border-b border-white/10 pb-8 sm:flex-row sm:items-center sm:justify-between">
        ...
        <div className="flex items-center gap-3">
          ...
          <Button variant="outline" type="button" onClick={() => void signOut({ callbackUrl: "/login" })}>
            退出
          </Button>
        </div>
      </header>
            ...
            <div className="flex flex-wrap gap-3">
              {latest.platforms.map((p) => (
                <a
                  ...
                  className={cn(
                    "inline-flex items-center rounded-lg border border-cyan-500/40 bg-cyan-500/10",
                    "px-4 py-2 text-sm font-medium text-cyan-100 hover:bg-cyan-500/20"
                  )}
                >
                  下载 {labelFor(p)}
                </a>
