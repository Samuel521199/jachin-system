...
export const nexusStore = {
  zh: {
    ...
    toastSubscribeOk: "订阅成功。",
    toastNetwork: "网络错误，请重试",
    toastLocalInstallStart: (name: string) => `正在拉取完整能力到本机：${name}`,
    toastLocalInstallOk: (name: string, count: number) => `已拉取到本机：${name}（含依赖 ${count} 个包）`,
    toastLocalInstallFallback: (name: string) => `浏览器拦截了直接拉取，已打开本机安装页：${name}`,
    toastLocalInstallFail: (reason: string) => `本机拉取失败：${reason}。请确认 Jachin/L3 已启动。`,
    toolBuiltinBadge: "L3 内置",
    toolPreinstalled: "已预装",
    toolBuiltinHint: "随终端运行时加载，无需订阅",
    mcpNeedsLocalModels: "需要本地小模型",
    mcpModelRecommendTitle: "推荐先安装以下本地模型（点击即自动下载）",
    modelInstallNow: "一键安装模型",
    modelInstalling: "下载中...",
    modelMissingPkg: "该模型暂未提供下载包，请先到“模型资产”订阅",
    modelGoAssetTab: "去模型资产",
    toastModelInstallStart: (name: string) => `正在安装模型到本机：${name}`,
    toastModelInstallOk: (name: string, path?: string) => `模型已安装到本机：${name}${path ? `（${path}）` : ""}`,
    toastModelDownloadFallback: (name: string) => `未检测到本机 L3，已改为下载安装包：${name}`,
    toastModelDownloadStart: (name: string) => `已开始下载模型：${name}`,
    toastModelDownloadFail: "模型下载失败，请稍后重试",
    toastModelNeedAsset: "该模型未提供下载包，请先在模型资产中获取",
  },
  en: {
    ...
    toastSubscribeOk: "Subscribed.",
    toastNetwork: "Network error — try again",
    toastLocalInstallStart: (name: string) => `Installing full capability locally: ${name}`,
    toastLocalInstallOk: (name: string, count: number) => `Installed locally: ${name} (${count} package(s) including dependencies)`,
    toastLocalInstallFallback: (name: string) => `The browser blocked direct local install. Opened the local installer page: ${name}`,
    toastLocalInstallFail: (reason: string) => `Local install failed: ${reason}. Make sure Jachin/L3 is running.`,
    toolBuiltinBadge: "L3 built-in",
    toolPreinstalled: "Pre-installed",
    toolBuiltinHint: "Ships with the runtime — no subscription",
    mcpNeedsLocalModels: "Needs local model(s)",
    mcpModelRecommendTitle: "Recommended local models (one click to download)",
    modelInstallNow: "Install model",
    modelInstalling: "Downloading…",
    modelMissingPkg: "No package URL yet. Subscribe from Model assets first.",
    modelGoAssetTab: "Go to Models",
    toastModelInstallStart: (name: string) => `Installing model locally: ${name}`,
    toastModelInstallOk: (name: string, path?: string) => `Model installed locally: ${name}${path ? ` (${path})` : ""}`,
    toastModelDownloadFallback: (name: string) => `Local L3 was not detected. Downloading package instead: ${name}`,
    toastModelDownloadStart: (name: string) => `Model download started: ${name}`,
    toastModelDownloadFail: "Model download failed. Please retry.",
    toastModelNeedAsset: "This model has no package URL yet. Get it from Model assets first.",
  },
} as const;
