 ...
 * 校验 Session 后 302 到 MinIO 预签名 URL（与 jachin-downloads 行为一致，共享同一 DB / Bucket）。
 */
export async function GET(request: NextRequest) {
  ...
  if (!isDesktopReleasesS3Configured()) {
    return NextResponse.json(
      { error: "对象存储未配置 DESKTOP_RELEASES_S3_*" },
      { status: 503 }
    );
  }
  ...
  if (!version || !platform) {
    return NextResponse.json({ error: "缺少 version 或 platform" }, { status: 400 });
  }
