"""PMO 调试日志：function calling 目的推断 + tool_calls Reasoning trace 合成."""

import json

from l3_node.llm_client import tool_calls_to_work_order_draft_text
...
def test_tool_calls_to_work_order_draft_uses_sql_inference() -> None:
    sql = "SELECT view_id, record_count FROM pmo_views_meta"
    calls = [{"function": {"name": "core_db_query", "arguments": json.dumps({"sql": sql})}}]
    out = tool_calls_to_work_order_draft_text(calls, openapi_fname_to_tool_id={"core_db_query": "core:db_query"})
    assert "（API function calling）" not in out
    assert "work_order_suggestion" in out
...
def test_tool_calls_use_assistant_content_thought() -> None:
    ...
    out = tool_calls_to_work_order_draft_text(
        calls,
        openapi_fname_to_tool_id={"core_db_query": "core:db_query"},
        assistant_content="Reasoning note: Step1·先读视图地图确认列名",
    )
    assert "Step1" in out
