"""core:db_query Action Input 解析（裸 SQL vs 畸形 JSON 包装）。"""
from __future__ import annotations

from l3_node.tools.pmo_db_tools import parse_db_query_action_input, run_db_query


def test_plain_sql_input():
    sql = "SELECT 1 AS x FROM pmo_raw_records LIMIT 1;"
    p = parse_db_query_action_input(sql)
    assert p.get("sql") == sql


def test_valid_json_wrapper():
    inp = '{"sql": "SELECT 1 AS x LIMIT 1;"}'
    assert parse_db_query_action_input(inp).get("sql") == "SELECT 1 AS x LIMIT 1;"
...
def test_malformed_json_with_embedded_quotes():
    ...
    sql = parse_db_query_action_input(inp).get("sql") or ""
...
def test_malformed_json_not_missing_sql_on_run():
    ...
    out = run_db_query(sql=parse_db_query_action_input(inp).get("sql", ""))
