...
- **界面事实**仅以最近一次 `mcp:get_holographic_screen` 的 Observation 为准：JSON `elements` + **带红框编号的标注图**。
...
2. **想**：Thought 中写明目标 id、可见文案（`content`）、为何点击。
3. **做**：`mcp:physical_click`，例如 `{"element_id": 12}`；打开桌面快捷方式时常用 `{"element_id": 3, "double_click": true}`。
4. **验**：界面变化后**必须**再次 `get_holographic_screen`，再 Final Answer。
