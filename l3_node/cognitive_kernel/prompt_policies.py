...
Role executors consume prompt snippets, but the policy source must live outside
``agent_core.py`` so the main agent does not keep growing into an architecture
document.
"""
...
2. map：基于 Verification evidence 和业务语义字典说明字段映射，不能跳过证据。
3. execute：只在输入、权限和风险等级允许时执行读写动作。
4. continuous execution：如果用户要求“先查再改”，必须连续完成读、判断、写、校验，不能在只完成查询后 User-facing result。
