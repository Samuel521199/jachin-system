"""Tool input adapter helpers for WorkOrder execution.

The Memory-first mainline executes through DecisionContract -> WorkOrder ->
Dispatcher -> RoleExecutor. These helpers normalize tool inputs and local
context without granting direct tool execution rights.
"""
    ...
    *,
    ...
    try:
            ...
            for key in ("sql", "query", "statement", "command"):
                ...
                if isinstance(value, str) and value.strip():
                    ...
                    logger.info(
                        ...
                        len(sql),
                        sql[:12000],
                        "...(truncated)" if len(sql) > 12000 else "",
                    )
                    return
            logger.info(
        ...
        logger.info(
            ...
            run_id,
            tool,
            (work_order_input or "")[:800],
            "...(truncated)" if len(work_order_input or "") > 800 else "",
        )
...
def prepare_lark_send_text_input(
    *,
    ...
    if (tool or "").strip() != "util:lark_send_text":
        return work_order_input, str(metadata.get("_lark_chat_id") or "")
    ...
    return work_order_input, lark_bind
