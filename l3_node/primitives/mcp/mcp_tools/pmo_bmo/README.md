...
| MCP ID | 模块 | 说明 |
|--------|------|------|
| `mcp:atom_pmo_lark_doc` | `tool_lark_doc.py` | `sync` / **`export_pmo_tables`**（四表 JSON→`~/.jachin/client_volumes/PMO/raw`，MD→`docs/pmo_bmo_plugin/raw`，DuckDB）/ `list_nodes` / `read_doc` |
