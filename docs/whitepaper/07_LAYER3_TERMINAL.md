...
│   └── tauri.conf.json
五、 v5.0 废弃声明 (Deprecation in v5.0)
❌ 废弃极客命令行输入: 彻底取消了要求用户在终端输入 api_key 或配置 .env 文件的反人类设定。

❌ 废弃本地 Dapr 端口映射: v4.0 中的本地 Pub/Sub 端口 (8002) 和 Dapr Sidecar 集成已被彻底移除。所有与云端的通信完全交由后台静默运行的 Layer 2 Daemon (HTTP 心跳) 接管，Layer 3 仅专注于 UI 呈现与进程存活监控。