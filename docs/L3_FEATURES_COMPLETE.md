...
预定义 `critic / executor / domain_expert` 等角色；`_build_system_prompt` 后缀注入 `delegate_hint`，让主 Agent 知道如何组建子 Agent 团队。

---

### F — 子 Agent 禁嵌套 delegate
**文件**：`l3_node/agent_core.py`  
子 Agent（`max_delegate_depth > 0`）尝试调用 `delegate_sub_agent` 时，直接返回 JSON 错误 Observation，防止无限嵌套导致栈爆炸。
...
- `agent_core` delegate 分支新增 `mode: discuss` 路由，Observation 前缀 `format_summary()` 摘要行
...
### AN — ReAct 内护栏 GuardrailsChecker
...
| `JACHIN_GUARDRAILS_ENABLE` | 0 | ReAct 内护栏（AN） |
