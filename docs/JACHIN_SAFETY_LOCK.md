...
- 历史字段 **`append_secret` / Action `token`**：**已废弃**；模型侧传入的 token **被忽略**，不得用于授权写正式文件（见 REMEDIATION 文档「密钥悖论」）。

### Action Input 示例（append）
...
- 未命中域时，可注入全局 `JACHIN_SAFETY_LOCK.md` 的 **头段**（长度由 `legacy_global_head_chars` 控制，`0` 可关闭）。
