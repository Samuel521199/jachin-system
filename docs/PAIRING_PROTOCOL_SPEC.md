...
| **Layer 1 动作** | 验证配对码有效 → 将 session 与 `user_id` 绑定 → 在 `layer2_instances` 中注册机甲身份 |
...
| | • `access_token` — 机甲拉取插件、发送心跳的专属通行证 |
| | • `layer1_public_key` — 验证 .jmp 插件包签名的公钥 |
| **机甲保存** | 写入本地 `.env` 或加密 `config.yaml` |
| **终端反馈** | `✅ 授权成功，机甲已连接至指挥部。` |
