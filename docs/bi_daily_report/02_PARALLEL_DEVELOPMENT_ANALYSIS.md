...
A 分支 feat/bi-scraper 仅包含：
  l3_node/primitives/mcp/mcp_tools/bi/tool_web_scraper.py  （新建）

B 分支 feat/bi-broadcaster 仅包含：
  l3_node/primitives/mcp/mcp_tools/bi/tool_lark_notifier.py、tool_email_sender.py  （新建）

C 分支 feat/bi-skill 仅包含：
  l3_node/primitives/skills/bi/bi_daily_report/main_skill.py
  l3_node/bi/scheduler.py                       （新建）

统帅分支（组装）修改：
  l3_node/primitives/mcp/registry.py  （追加路由与注册）
  l3_node/__main__.py 或 http_server.py  （可选：import l3_node.primitives.skills.bi.scheduler）
```
...
| **l3_node/primitives/mcp/mcp_tools/__init__.py** | A、B 首次创建 `mcp_tools/` 目录时，可能都添加 `__init__.py` | **规范**：统帅在项目启动前**预创建** `l3_node/primitives/mcp/mcp_tools/` 目录及空 `__init__.py`，A/B 仅添加各自 py 文件 |
...
```
阶段 0：统帅预备（1 天）
  ├─ 创建 l3_node/primitives/mcp/mcp_tools/ 目录及 __init__.py
...
阶段 3：统帅组装
  ...
  ├─ 在 __main__.py 或 http_server 中 import l3_node.primitives.skills.bi.scheduler
...
- 或：在 `l3_node/primitives/mcp/mcp_tools/` 下新增 `requirements.txt`（仅 BI 工具用），由 A、B 分别追加自己需要的包，合并时可能冲突，需统帅协调。
...
| 序号 | 检查项 | 负责方 |
|------|--------|--------|
| 1 | 统帅预创建 `l3_node/primitives/mcp/mcp_tools/` 及 `__init__.py` | 统帅 |
