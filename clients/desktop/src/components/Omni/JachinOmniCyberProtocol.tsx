...
function OmniHologramCorners() {
  const c =
    "pointer-events-none absolute z-[1] h-3 w-3 border-sky-200/[0.18] [box-shadow:0_0_12px_rgba(125,211,252,0.12)]";
  return (
    <>
...
export const OmniCyberChatShell: React.FC<OmniCyberChatShellProps> = ({
  ...
  return (
    <div
      className={`omni-command-shell relative flex h-full min-h-0 w-full min-w-0 flex-col overflow-visible p-2 pointer-events-none sm:p-3 ${riskBorder} ${clipMain}`}
      style={{ background: "transparent" }}
    >
      <div
        className={`omni-command-inner relative flex h-full min-h-0 w-full min-w-0 flex-col overflow-hidden pointer-events-auto ${clipMain}`}
      >
        <div
          className="pointer-events-none absolute inset-0"
          style={{
            background:
              "radial-gradient(ellipse 85% 55% at 50% 0%, rgba(56, 189, 248, 0.08), transparent 58%), linear-gradient(180deg, rgba(8, 14, 25, 0.84), rgba(4, 8, 15, 0.78))",
            backdropFilter: "blur(22px) saturate(1.08)",
            WebkitBackdropFilter: "blur(22px) saturate(1.08)",
          }}
        />
        <div className="pointer-events-none absolute inset-0 shadow-[inset_0_1px_0_rgba(255,255,255,0.04),inset_0_0_68px_rgba(56,189,248,0.045),0_24px_68px_rgba(0,0,0,0.46)]" />
        ...
        <div className="relative z-10 flex h-full min-h-0 min-w-0 flex-col overflow-hidden">
          ...
          <div className="omni-topbar flex min-w-0 shrink-0 select-none items-center justify-between gap-1 px-3 py-2 sm:gap-2 sm:px-4">
            <div
              data-tauri-drag-region
              className="flex min-w-0 flex-1 items-center gap-1.5 sm:gap-2"
            >
              <span className="pointer-events-none text-[10px] font-semibold uppercase tracking-[0.24em] text-sky-200/[0.85]">
                Omni
              </span>
              {onToggleSessionDrawer != null ? (
                <button
                  ...
                  className="omni-icon-button pointer-events-auto flex h-7 w-7 shrink-0 items-center justify-center active:scale-95"
                >
              ...
              <div className="pointer-events-none flex min-w-0 flex-1 justify-center px-2">
                <span className="h-px w-16 shrink-0 rounded-full bg-gradient-to-r from-transparent via-sky-300/[0.24] to-transparent" aria-hidden />
              </div>
            </div>
            <div
          ...
          <div className="relative min-h-0 min-w-0 flex-1 overflow-x-hidden overflow-y-auto overscroll-y-contain">
            <div
              className={`relative z-10 flex min-h-full min-w-0 max-w-full flex-col gap-4 px-3 py-4 transition-[filter,opacity] duration-300 sm:px-5 ${
              ...
              {messages.map((msg, idx) => {
                  ...
                  return (
                    <div key={`${msg.timestamp}-${idx}`} className="flex min-w-0 justify-end gap-2">
                      <motion.div
                        ...
                        className="omni-msg omni-msg-user relative min-w-0 max-w-[min(100%,42rem)] px-3 py-2.5 text-sm sm:px-4"
                      >
                        <div className="relative z-[2] min-w-0 max-w-full break-words whitespace-pre-wrap [overflow-wrap:anywhere] leading-relaxed">
                          {msg.content}
                        </div>
                      </motion.div>
                      <div className="hidden w-8 shrink-0 flex-col items-center pt-1 sm:flex">
                        <div className="h-full min-h-[2rem] w-px shrink-0 bg-gradient-to-b from-transparent via-sky-300/[0.18] to-transparent" />
                        <span className="mt-1 font-mono text-[7px] leading-none text-sky-500/35">{tel}</span>
                      </div>
                    </div>
                  );
                }
                if (msg.role === "assistant") {
                  ...
                  return (
                    ...
                    >
                      <div className="hidden w-8 shrink-0 flex-col items-center pt-1 sm:flex">
                        <div className="h-full min-h-[2rem] w-px shrink-0 bg-gradient-to-b from-transparent via-emerald-300/[0.16] to-transparent" />
                        <span className="mt-1 font-mono text-[7px] leading-none text-sky-500/30">{tel}</span>
                      </div>
                      <div className="omni-msg omni-msg-assistant relative min-w-0 flex-1 max-w-[min(100%,52rem)] px-3 py-3 text-sm sm:px-4">
                        <div className="relative z-[2] min-w-0 max-w-full">
                          ...
                          <div className="min-w-0 max-w-full break-words leading-relaxed text-sky-50/[0.94] [overflow-wrap:anywhere] [&_.markdown-content]:font-medium">
            ...
            {(recordingStatus || listeningText || isVadActive) && (
              <div className="relative z-20 mx-3 mb-1 flex min-w-0 flex-col gap-1 break-words text-[10px] [overflow-wrap:anywhere] sm:mx-5">
                {isVadActive && (
                  <div className="rounded-lg border border-amber-300/20 bg-amber-400/[0.08] px-2 py-1 text-amber-100/[0.85] shadow-[0_0_16px_rgba(245,158,11,0.08)]">
                    {ui.vadListening}
                  </div>
                )}
                {recordingStatus && (
                  <div
                    className={
                      /错误|error/i.test(recordingStatus) ? "text-red-300/95" : "text-cyan-300/[0.85]"
                    }
                  >
                    {recordingStatus}
                  </div>
              ...
              {hitlPending && (
                <>
                  <motion.div
                    ...
                    className="pointer-events-auto absolute inset-0 z-30 bg-slate-950/[0.48] backdrop-blur-md"
                  />
                  <motion.div
                    key="hitl-console"
                  ...
                  >
                    <div className="omni-hitl-card pointer-events-auto w-full max-w-md p-5">
                      <p className="mb-2 text-center text-[11px] font-semibold uppercase tracking-[0.28em] text-amber-100/95 [text-shadow:0_0_14px_rgba(251,191,36,0.22)]">
                        {ui.hitlTitle}
                      </p>
                      <p className="mb-4 max-h-32 overflow-y-auto whitespace-pre-wrap rounded-lg border border-amber-300/[0.18] bg-amber-400/[0.045] px-3 py-2 font-mono text-xs leading-relaxed text-slate-100/[0.92] shadow-[inset_0_0_20px_rgba(251,191,36,0.035)]">
                        {hitlPending.content || ui.hitlFallback}
                      </p>
                      <div className="flex justify-center gap-3">
                        <button
                          type="button"
                          className="omni-primary-button px-5 py-2.5 text-sm font-medium"
                        ...
                        <button
                          type="button"
                          className="omni-secondary-button px-5 py-2.5 text-sm"
          ...
          <motion.div
            ...
            className="relative z-20 mx-2 mb-3 mt-1 min-w-0 shrink-0 sm:mx-4 sm:mb-4"
            style={{ pointerEvents: "auto" }}
          >
            <div className="omni-input-dock relative isolate overflow-hidden">
              <motion.div
                aria-hidden
                className="pointer-events-none absolute inset-0 z-0 opacity-55 will-change-transform"
                style={{
                  background:
                    "conic-gradient(from 0deg, transparent 0deg, transparent 292deg, rgba(125,211,252,0.10) 306deg, rgba(56,221,245,0.34) 314deg, rgba(85,240,178,0.20) 322deg, transparent 338deg, transparent 360deg)",
                }}
                animate={{ rotate: [0, 360] }}
                transition={{ duration: 22, repeat: Infinity, ease: "linear" }}
              />
              <div className="omni-input-dock-inner relative z-[1] m-px min-h-0 px-2 pb-2 pt-1.5 sm:px-2.5 sm:pb-2.5 sm:pt-2">
            <input
                  ...
                  pendingFiles.map((file, idx) => {
                    ...
                    return (
                      <div
                        ...
                        className={`group relative inline-flex max-w-[11rem] items-center gap-1.5 rounded-lg border border-sky-300/10 bg-sky-300/[0.035] py-1 pl-2 pr-6 text-[10px] text-sky-100/90 shadow-[0_8px_28px_rgba(0,0,0,0.22)] ${
                          pulseClipboard
                ...
                {voiceVisual ? (
                  ...
                  <div className="relative">
                    <textarea
                      ...
                      autoComplete="off"
                      spellCheck={false}
                      className="omni-command-textarea w-full min-h-[48px] min-w-0 max-h-[200px] resize-none overflow-y-auto break-words border-0 bg-transparent px-2 py-2 text-sm leading-relaxed focus:outline-none focus:ring-0 disabled:opacity-50 [overflow-wrap:anywhere] [scrollbar-width:none] [-ms-overflow-style:none] sm:px-3 sm:py-2.5 [&::-webkit-scrollbar]:hidden"
                    />
                  </div>
                )}
              </div>
              ...
              <div className="flex shrink-0 flex-col justify-end gap-1 pb-1 sm:gap-1.5">
                {!voiceVisual &&
                  (stopMode ? (
                    <button
                      ...
                      className="omni-icon-button flex h-9 w-9 shrink-0 items-center justify-center text-violet-200/95"
                    >
                    ...
                    <button
                      ...
                      disabled={!canSend}
                      title="发送"
                      className={`omni-send-button flex h-9 w-9 shrink-0 items-center justify-center ${
                ...
                {onOpenConsole && (
                  <button
                    ...
                    className="omni-icon-button flex h-9 w-9 shrink-0 items-center justify-center"
                  >
