...
`npx` 包名已固定为 `@modelcontextprotocol/server-filesystem@0.6.2`（须为 npm 已发布版本），与 `mcp` Python SDK 1.26+ 及 `tools/list` 参数序列化行为对齐；若你仍用无版本号的 `-y @modelcontextprotocol/server-filesystem`，可能拉到不兼容版本。
