#!/usr/bin/env node
/**
 * 基于 @modelcontextprotocol/server-puppeteer（MIT）的 Jachin 分支：
 * 若设置环境变量 PUPPETEER_BROWSER_URL（或 JACHIN_PUPPETEER_BROWSER_URL），
 * 则使用 puppeteer.connect({ browserURL }) 附加到已有 Chrome（如 --remote-debugging-port=9222），
 * 不再 puppeteer.launch 新窗口。未设置时行为与上游一致（launch）。
 */
...
function getConnectBrowserURL() {
    return (process.env.PUPPETEER_BROWSER_URL || process.env.JACHIN_PUPPETEER_BROWSER_URL || '').trim();
}
function attachPageConsoleHandlers(p) {
    p.on("console", (msg) => {
...
async function pickInitialPage(br) {
    ...
    return preferred || pages[0] || await br.newPage();
}
function browserLooksConnected() {
    if (!browser)
        return false;
    try {
...
async function ensureBrowser({ launchOptions, allowDangerous }) {
    const connectUrl = getConnectBrowserURL();
    if (connectUrl) {
        if (!browserLooksConnected()) {
            await teardownBrowser();
            browser = await puppeteer.connect({ browserURL: connectUrl, defaultViewport: null });
            page = await pickInitialPage(browser);
            attachPageConsoleHandlers(page);
        }
        return page;
    }
...
const server = new Server({
    name: "jachin-servers/puppeteer-cdp",
    version: "2025.5.12+jachin.cdp1",
}, {
    capabilities: {
        resources: {},
        tools: {},
