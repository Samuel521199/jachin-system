...
- **发帖** = 仅 `atom_post_job_boss`；`boss_post_published=true` 后默认**不可再发帖**，除非 HR 明确重新发布并 **`force_republish: true`**。
- **打招呼/收网/透析** = 只动 jd.json 调度字段 + `hr_scheduler_send_confirm_prompt` / `add_automated_recruitment_task` / 飞书指令；**禁止**为改数字再发帖。
- 飞书上线简报 ≠ 要再发帖；调度确认单 ≠ 发帖。

- **分支 A（新发布）**：在 HR 明确回复「同意」或点击确认之前，**绝对禁止**调用 `atom_post_job_boss`、`hr_scheduler_send_confirm_prompt` 与 `add_automated_recruitment_task`。**首次发帖成功后**：必须先 `hr_scheduler_send_confirm_prompt`，飞书 **「同意调度」** 后才注册定时任务。**已发帖后**若只改调度参数，**禁止**再输出「请确认发布」整卡。
...
- 回复 HR：已启动无人值守，并简要复述：**是否打招呼、收集上限、是否自动透析**。
...
职位发布成功后，**先** `mcp:hr_scheduler_send_confirm_prompt`（至少 `job_name`；可选 `greet_harvest_switch_interval_minutes`、`greet_target`、`max_count_per_harvest_tick`、`analyze_threshold`、`resume_collect_target` 等）写入 `jd.json` 并发飞书参数单；默认 **牛人沟通 → 抓简历** 严格轮换，**无**「打满人再等 20 秒收网」。飞书 **「同意调度」** 后注册；或 `mcp:add_automated_recruitment_task`。满 `analyze_threshold` 份触发透析。手工分析：**mcp:hr_analyze_resume**。
...
告知：职位已上线；飞书已发 **调度参数确认单**，**定时任务未启动**，请飞书回复 **「同意调度」**；无飞书则让助手执行 `add_automated_recruitment_task`。

## 飞书遥控（进度与继续）

- **抓取进度**：无人值守收网每轮 tick 结束后，若已配置 `LARK_APP_ID`/`LARK_APP_SECRET` 且存在 **`LARK_CHAT_ID` 环境变量** 或 HR 曾在本群发过消息（系统已写入指针中的 `lark_chat_id`），会向该会话推送 **`📊 【岗位】抓取简历 n/m 份`**（`m` 为本次任务的 `resume_collect_target`）。
- **停止**（短句「停止/暂停」等）：高优指令会注入 `STOP_HARVEST`，当前 Playwright 长任务在探针处停下；回复中会附带**当前进度**。
- **继续**：HR 发 **「继续」「继续收网」「恢复」** 等（不含单独「继续分析」歧义）时，会清除全局停止标志与 `STOP_HARVEST`，若定时任务已被撤掉则按**上次持久化的 `job_name` 配置**自动重新 `add_automated_recruitment_task`；回复中带**当前进度**。
