...
MCP 暴露的工具名以当前轮 **可用工具列表**为准，常见为 **`get_video_info`**、**`get_video_comments`**（调用时的 **Action** 多为 **`mcp:get_video_info`** / **`mcp:get_video_comments`**）。
...
若在可用工具列表中**找不到**上述 **`mcp:get_*`** 且无任何可替代的同源 B 站信息工具：在 Final Answer 中说明 **`工具未挂载成功`**，并提示检查 **`bilibili-mcp`** 是否已写入 **`~/.jachin/mcp_servers.json`**、Node/npx 是否可用、是否已重启 L3。**禁止**编造字幕或评论。
