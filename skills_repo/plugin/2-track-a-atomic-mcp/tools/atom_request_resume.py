...
支持 jd_config_path：从 data/{岗位名}/jd.json 读取岗位配置，与 atom_post_job_boss 一致。
"""
import logging
from pathlib import Path

from .atom_post_job_boss import load_jd_config, get_jd_select
from .boss_utils import navigate_to_candidate_chat, navigate_to_chat_page, select_job
from .human_utils import human_wait

logger = logging.getLogger(__name__)


def _job_keyword_from_jd(jd: dict) -> str:
    """从 JD 配置提取职位下拉匹配关键词（job_keywords[0] 或 job_title）"""
    kw = jd.get("job_keywords")
    if kw and isinstance(kw, list) and len(kw) > 0:
        return str(kw[0]).strip() or (jd.get("job_title") or "").strip()
    return (jd.get("job_title") or "").strip()
...
def atom_request_resume(
    ...
    Args:
        ...
        jd_config_path: data/{岗位名}/jd.json 路径；传入时从中读取 job_keyword，与 atom_post_job_boss 一致
        job_keyword: 职位关键词（全部职位下拉匹配）；若 jd_config_path 有效则被其覆盖
        candidate_name: 目标候选人姓名
    ...
    if jd:
        # 优先用 jd_select 精确匹配 Boss「全部职位」下拉选项；否则用 job_keyword
        job_select = get_jd_select(jd)
        job_keyword = (job_select or job_keyword or "").strip() or _job_keyword_from_jd(jd)
    if not (job_keyword or "").strip():
        return {"success": False, "request_sent": False, "error": "请传入 job_keyword 或 jd_config_path（指向 data/{岗位名}/jd.json）"}
    try:
...
def atom_request_resume_batch(
    ...
    1. 选择指定岗位 JD（job_text 或从 jd_config_path 指向的 data/{岗位名}/jd.json 读取）
    2. 遍历列表中的对话项，逐个点击进入
    ...
    Args:
        jd_config_path: data/{岗位名}/jd.json 路径；传入时从中读取 job_text，与 atom_post_job_boss 一致
        job_text: Boss「全部职位」匹配文本（如「资深Golang语言开发 25-40K」）；jd_config_path 有效时被其覆盖
    ...
    jd = load_jd_config(jd_config_path, job_name)
    if jd:
        # 优先用 jd_select 精确匹配 Boss「全部职位」下拉（沟通页：span.chat-select-job）
        job_text = (job_text or "").strip() or get_jd_select(jd)
    if not (job_text or "").strip():
        return {
            ...
            "processed": [],
            "error": "请传入 job_text 或 jd_config_path（指向 data/{岗位名}/jd.json）",
        }
    try:
        from playwright.sync_api import sync_playwright
    except ImportError:
    ...
    try:
        with sync_playwright() as pw:
            ...
            # 若在个人中心等非沟通页，先安全跳转到沟通页（避免 select_job 误点右上角头像）
