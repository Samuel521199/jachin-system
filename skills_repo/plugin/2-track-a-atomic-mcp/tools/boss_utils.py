...
# 职位搜索输入框（Boss 新 UI「请输入职位名称」），优先填充以便筛选
BOSS_JOB_SEARCH_INPUT_SELECTORS = [
    "input.chat-job-search",
    "input[placeholder*='职位']",
    "input[placeholder*='请输入']",
]
# 职位筛选：Boss 新 UI 为搜索框，老 UI 为下拉。仅用主内容区选择器，不包含可匹配头像的泛用选择器
...
def _to_boss_search_term(raw: str, full_job_text: str) -> str:
    """将 JD 职位名转换为 Boss 搜索框可匹配的词汇。Boss 列表用「高级golang开发工程师」，搜「Go后端工程师」会没有相关职位"""
    r, f = (raw or "").strip().lower(), (full_job_text or "").lower()
    # Go/Go后端/Go开发 → golang（Boss 展示为 golang）
...
def select_all_positions(page) -> bool:
    """
    点击「全部职位」下拉后，直接选择「全部职位」选项（不筛选具体职位）。
    用于收网抓简历时展示所有职位的会话，便于新职位在短时间内完成测试。
    """
    try:
        page.bring_to_front()
    ...
    # 点击下拉触发（div.ui-dropmenu-label 或 span.chat-select-job）
    try:
        for trigger_sel in [
            "div.ui-dropmenu-label:has(span.chat-select-job)",
            "span.chat-select-job",
...
def select_job(page, job_text: str) -> bool:
    """
    极简逻辑：输入关键词 → 点下拉第1项 → 不成点第2项 → 都不成则失败。不循环不复杂匹配。
    """
    ...
    human_wait(page, 0.8, 1.5)  # 等待下拉刷新
...
# 用户菜单/头像区域关键词，select_job 使用 div.ui-dropmenu-label 时必须排除
_USER_MENU_EXCLUDE_KEYWORDS = (
    "周", "个人中心", "退出", "账号", "钱包",
    "招聘者", "女士", "先生", "升级", "VIP", "我的客服",
    "创作中心", "地址管理", "招聘规范", "招聘数据", "账号权益",
)
# 职位选择框应有的特征（含 K 表示薪资、工程师/开发 表示职位，全部职位 为占位）
_JOB_LIKE_KEYWORDS = ("K", "工程师", "开发", "全部职位", "Java", "Golang", "杭州", "北京", "上海")


def _is_chat_inbox_page(url: str) -> bool:
    """判断当前是否在沟通（收件箱）页面：含 /geek/chat 或 /chat/index，排除 推荐/recommend、职位管理/job/list"""
    u = (url or "").lower()
...
def _is_user_menu_dropdown_open(page) -> bool:
    """检测是否误点了右上角头像，用户菜单已展开（含个人中心、退出等）"""
    try:
...
def navigate_to_chat_page(page) -> bool:
    """
    若当前不在沟通收件箱页，则安全跳转。优先点左侧「沟通」菜单（避免 get_by_text 误点右上角头像）。
    若检测到误点头像导致用户菜单展开，会先关闭再点击沟通。
    返回 True 表示已在沟通页或跳转成功。
    """
    try:
        ...
        if _is_chat_inbox_page(url):
            ...
            return True
        # 优先用 ka 属性点左侧菜单（与 推荐牛人 menu-geek-recommend 同源）
