...
def _extract_pdf_url_from_viewer(pages) -> str | None:
    """从 PDF.js viewer iframe 的 file= 参数提取 PDF 真实下载 URL。"""
    for page in pages:
        try:
            iframes = page.locator('iframe[src*="viewer.html"][src*="file="]')
            cnt = iframes.count()
            for i in range(cnt):
                src = iframes.nth(i).get_attribute("src")
                if not src or "file=" not in src:
                    continue
                parsed = urlparse(src)
            ...
            for frame in p.frames:
                for sel in BOSS_DOWNLOAD_SELECTORS:
                    try:
                        ...
                        if loc.count() > 0:
                            with context.expect_download(timeout=12000) as download_info:
                            ...
                            if content and len(content) >= 100:
                                logger.info("策略3 点击下载：成功，%d 字节", len(content))
                                return content, ""
                    except Exception:
                        pass
...
def download_resume_from_preview_page(
    ...
    Args:
        ...
        download_to_pending: 是否保存到 data/pending/
...
def atom_inbox_harvester_full_flow(
    ...
    save_dir: str | Path | None = None,
    filter_tab: str = "",
    request_if_no_resume: bool = False,
) -> dict:
    """
    完整流程：在「全部职位」中选择对应职位 → 遍历左侧所有候选人会话 →
    若有「点击预览附件简历」按钮则执行 PDF 下载；
    若无简历且 request_if_no_resume=True，则点击「求简历」向求职者索要简历。
    ...
    try:
        with sync_playwright() as pw:
            ...
            breath_interval = random.randint(3, 5)
            for i in range(n):
                try:
                    ...
                    try:
                        ...
                        cand_dir = Path(save_dir) / subdir if save_dir else None
                    ...
                    if out.get("success") and out.get("pdf_path"):
                        ...
                        if downloaded > 0 and downloaded % breath_interval == 0:
                            rest_sec = random.uniform(10.0, 20.0)
                            logger.info("[Anti-Bot] 模拟人类阅读疲劳，休息 %.1f 秒...", rest_sec)
