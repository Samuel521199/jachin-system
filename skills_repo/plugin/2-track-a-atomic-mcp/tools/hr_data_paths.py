...
PLUGIN_DATA_ROOT = _ROOT / "data"

# 全局 JD 模板：每次与 HR 沟通得到的职位配置均以此为基础填写
JD_TEMPLATE_PATH = PLUGIN_DATA_ROOT / "jd_to_publish.example.json"
...
def get_job_processed_dir(job_name: str) -> Path:
    """已对比简历 data/{job_folder}/processed/"""
...
def get_job_result_dir(job_name: str) -> Path:
    """AI 分析报告 data/{job_folder}/result/"""
...
def get_job_summary_md_path(job_name: str) -> Path:
    """职位专属排行榜 MD data/{job_folder}/排行榜_Summary.md"""
...
def init_job_jd_from_template(job_name: str, overrides: dict | None = None) -> Path:
    """
    HR 确认后自动执行：复制 jd_to_publish.example.json 到 data/{职位}/jd.json，用 overrides 覆盖填写并保存。
    ...
    # 1. 创建 pending、processed、result 子目录
    (base / "pending").mkdir(parents=True, exist_ok=True)
    (base / "processed").mkdir(parents=True, exist_ok=True)
    (base / "result").mkdir(parents=True, exist_ok=True)
    # 2. 复制模板到 jd.json，再根据 HR 确认内容覆盖填写
    ...
    # 自动补全 jd_select（Boss「全部职位」匹配用），若缺失则根据 job_title、salary 推导
    ...
    # 3. 创建职位专属输出 md 占位（排行榜_Summary.md）
