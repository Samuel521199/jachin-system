...
_JACHIN_ROOT = Path(os.environ.get("JACHIN_HOME", str(Path.home() / ".jachin"))).expanduser().resolve()


def _get_resolve_base() -> Path:
    """路径解析基准：订阅模式 ~/.jachin，开发模式项目根"""
    try:
        from l3_node.paths import get_app_root
        proj = get_app_root()
        if proj and proj.exists() and (proj / "l3_node").exists():
            return proj
    except Exception:
        pass
    return _JACHIN_ROOT
...
def _get_default_output_root() -> Path:
    """默认分析输出根：优先 ~/.jachin/workspace/hr_analysis"""
    hr_root = Path(__file__).resolve().parent
    if (hr_root / "tools" / "config.py").exists():
        import sys
        cache_str = str(hr_root.resolve())
        if cache_str not in sys.path:
            sys.path.insert(0, cache_str)
        try:
            from tools.config import get_hr_analysis_output_root
            return get_hr_analysis_output_root()
        except Exception:
            pass
    return _get_resolve_base() / "data" / "hr_analysis"


_PROJ_ROOT = _get_resolve_base()


def get_resolve_base() -> Path:
    """供 recruitment_task 等调用，获取路径解析基准"""
    return _get_resolve_base()
...
def persist_hr_analysis_result(
    ...
    # 兜底：~/.jachin/workspace/hr_analysis 或项目 data/hr_analysis 与 volume
    out_dirs.append(_get_default_output_root())
