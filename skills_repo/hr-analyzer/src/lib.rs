//! HR 简历透视镜 - Jachin OS L3 杀手级 Wasm Skill
//!
//! 流程：解析输入 → MCP 拉取简历 → 组装 Prompt → LLM 分析 → 附录倒置渲染
//! Host 函数：mcp_read_file(path_ptr, path_len), llm_complete(prompt_ptr, prompt_len)
//!
//! 编译: cargo build --target wasm32-unknown-unknown --release
//! 输出: target/wasm32-unknown-unknown/release/hr_analyzer.wasm -> main.wasm
