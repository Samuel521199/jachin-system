#!/usr/bin/env python3
"""
单页测试：平台产销情况 — 验证树形展开抓取（全部汇总 -> 奖励/赠送/游戏输赢 -> 各游戏）

Prereq: 1) .\scripts\launch_chrome_debug_bi.ps1  2) 在 Chrome 中登录 BI 后台
Usage:
  python scripts/test_bi_prod_sales.py          # 通过菜单导航
  python scripts/test_bi_prod_sales.py --direct  # 直接打开目标页（推荐）
Output: ~/.jachin/client_volumes/bi_data/raw/prod_sales.csv
"""
...
logger = logging.getLogger("test_prod_sales")

# 平台产销情况直接 URL（biGameDailyAssetSummary）
PROD_SALES_DIRECT_URL = "https://bi-admin-web.heronpro.xin/#/layout/BIManager/PlatformData/PlatformAsset/biGameDailyAssetSummary"
...
def main() -> int:
    ...
    out_path = raw_dir / "prod_sales.csv"

    use_direct = "--direct" in sys.argv

    if use_direct:
        logger.info("使用 --direct 模式，直接打开: %s", PROD_SALES_DIRECT_URL)
        r = harvest_table_data(
            url=PROD_SALES_DIRECT_URL,
            ...
            config={
                ...
                "automation": {
                    "start_url": PROD_SALES_DIRECT_URL,
                    "actions": [],  # 无菜单操作，直接等表格
        ...
        if r.get("status") == "success":
            logger.info("[1/1] prod_sales -> %s", r)
        else:
            logger.warning("[1/1] prod_sales -> %s", r)
    else:
    ...
    if out_path.exists():
        ...
        for kw in ["全部汇总", "奖励", "赠送", "游戏输赢", "Crazy Solitaire", "Mines Clash"]:
            if kw in content:
                print(f"  [OK] 含: {kw}")
            else:
                print(f"  [--] 未抓到: {kw}")
