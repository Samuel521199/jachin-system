#!/usr/bin/env python3
"""
单页测试：游戏数据统计对比 — 验证逐行「展开→提取→折叠」抓取

页面特点：展开「当日总计」会占满整页，无法点击其他日期行，需逐行展开→提取→折叠。
Prereq: 1) .\scripts\launch_chrome_debug_bi.ps1  2) 在 Chrome 中登录 BI 后台
Usage:
  python scripts/test_bi_stats_game_compare.py          # 通过菜单导航
  python scripts/test_bi_stats_game_compare.py --direct  # 直接打开目标页（推荐）
Output: ~/.jachin/client_volumes/bi_data/raw/stats_game_compare.csv
"""
...
logger = logging.getLogger("test_stats_game_compare")

# 游戏数据统计对比直接 URL
STATS_GAME_COMPARE_DIRECT_URL = "https://bi-admin-web.heronpro.xin/#/layout/BIManager/DataS/GameDataS/biGameDailySummaryCompare"
...
def main() -> int:
    ...
    out_path = raw_dir / "stats_game_compare.csv"
    ...
    if use_direct:
        ...
        logger.info("使用 --direct 模式，直接打开: %s", STATS_GAME_COMPARE_DIRECT_URL)
        automation = get_automation_for_direct_url("stats_game_compare", STATS_GAME_COMPARE_DIRECT_URL)
        r = harvest_table_data(
            url=STATS_GAME_COMPARE_DIRECT_URL,
        ...
        if r.get("status") == "success":
            logger.info("[1/1] stats_game_compare -> %s", r)
        else:
            logger.warning("[1/1] stats_game_compare -> %s", r)
    else:
        from l3_node.mcp_tools.bi.spa_collector import run_full_spa_collect

        ok, fail, _ = run_full_spa_collect(
            slugs=["stats_game_compare"],
            use_discover=False,
            auto_ingest=False,
            raw_dir=raw_dir,
