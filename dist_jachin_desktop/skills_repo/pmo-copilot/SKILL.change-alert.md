...
2. Python → `build_change_alert_prose_brief`（**BI 大战报结构**：🎯定调 → 📋变更 → ⚠️影响 → 💡建议）
3. **LLM**（可选）→ 读压缩版 `analysis_brief` 润色为同类文风；失败则用规则 prose
4. 标题：`human_change_alert_title`
5. **禁止**在正文末尾输出 `change_alert_result`（仅日志）

| 环境变量 | 含义 |
|----------|------|
| `PMO_CHANGE_ALERT_LLM_NARRATE=0` | 不用 LLM，仅规则大战报 prose |
| `PMO_CHANGE_ALERT_TECHNICAL=1` | 调试：旧版 GFM 表格 |

**禁止** LLM 自由查库改结论；**禁止**需求进度全览 / 版本映射三表；**禁止**「→ 字段列表」式堆叠输出。
